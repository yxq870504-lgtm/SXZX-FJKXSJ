...
<div class="shell">
  ...
  <div class="toolbar-card"><div class="toolbar">
    ...
    <div class="pill grade-pill" id="gradeTabs"></div>
...
function renderGradeTabs(){const tabs=document.getElementById('gradeTabs'); tabs.innerHTML=['全部',...grades()].map(g=>`<button data-grade="${g}" class="${gradeFilter===g?'active':''}">${g}</button>`).join(''); tabs.querySelectorAll('button').forEach(b=>b.onclick=()=>{gradeFilter=b.dataset.grade; render();});}
function render(){recompute();renderGradeTabs();updateKpi();renderTables('holiday','holidayDist','holidayAgg','寒假放假时间');renderTables('spring','springDist','springAgg','春季开学时间');renderBars('holidayBars','holiday');renderBars('springBars','spring','purple');renderTop5();renderGrades()}
...
document.querySelectorAll('#stageTabs button').forEach(x=>x.classList.remove('active'));b.classList.add('active');stage=b.dataset.stage;gradeFilter='全部';render();});
render();
</script>
</body>
