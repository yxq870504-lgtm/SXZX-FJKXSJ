...
function ensureChinaMap(){if(!window.echarts)return Promise.resolve(false); if(chinaMapReady)return chinaMapReady; chinaMapReady=fetch('data/china-mainland-trimmed.json?v=mainland0811').then(r=>r.json()).then(geo=>{echarts.registerMap('china-mainland',geo); return true;}).catch(()=>false); return chinaMapReady;}
