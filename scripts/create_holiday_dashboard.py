    ...
    <div class="panel" style="margin-top:18px">
      ...
      <div class="map-summary-grid">
        ...
        <aside class="summary-card">
          <div class="summary-eyebrow">当前年级省份分布</div>
          <div class="top5-copy" id="rankCopy">在班学员所在省份top5：</div>
          <div class="summary-tip">鼠标悬停地图省份，可查看该省份在班人数与占比。</div>
...
async function renderMap(){
  ...
  document.getElementById('rankCopy').innerHTML=`<span>${grade}</span>在班学员所在省份Top5</span><div class="top5-bars">${top5.map(x=>`<div class="top5-bar-row"><div class="top5-name">${x.short}</div><div class="top5-track"><div class="top5-fill" style="width:${Math.max(4, max ? x.count / max * 100 : 0)}%"></div></div><div class="top5-count">${fmtNum(x.count)}</div></div>`).join('')}</div>`;
